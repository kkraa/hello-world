...
curl -k -s -H "Authorization: Bearer $token" -X POST -F "jobDefinitionsFile=@testjobs.json" "$endpoint/deploy"
submit=$(curl -k -s -H "Authorization: Bearer $token" -X POST -F "jobDefinitionsFile=@testjobs.json" "$endpoint/run")
