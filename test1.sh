...
curl -k -s -H "Authorization: Bearer $token" -X POST -F "jobDefinitionsFile=@testjobs1.json" "$endpoint/deploy" 
submit=$(curl -k -s -H "Authorization: Bearer $token" -X POST -F "jobDefinitionsFile=@testjobs1.json" "$endpoint/run") 
